...
## 10. Hypothesis Ranking — Only When the Code Genuinely Supports It

**The change:** the investigator's existing single-pass task (already consolidated from 3 agents to 2 earlier, specifically to avoid a retrieval→hypothesis hand-off seam) now states up to 3 ranked hypotheses, each with its own confidence, *if and only if* the code it actually read genuinely supports more than one explanation. The fix agent still proposes exactly one fix, against the highest-confidence hypothesis — ranking exists for the human reviewer's context ("here's what else we considered"), not to generate multiple competing fixes. The single-proposal, human-reviews model stays unchanged.

**Caught directly mid-build: don't let the investigator's prompt keep growing unchecked.** First draft of the ranking instruction was a full paragraph repeating constraints already implied elsewhere. Flagged directly — the investigator already does a lot in one pass (list files, read files, cross-reference, diagnose, cite real names only, state confidence, quote lines, and now optionally rank), and growing the prompt further deserved real scrutiny, not just being waved through because the addition itself seemed reasonable. Considered and rejected splitting ranking into a separate agent — ranking isn't a distinct cognitive step from diagnosing (you can't rank hypotheses without already having done the diagnosis), so splitting it would recreate the exact hand-off-seam problem that caused the original 3-agent hallucination bug, for no real benefit. Trimmed the actual addition to two sentences instead.

**Verified the normal case doesn't regress, deliberately not chasing proof that ranking itself fires.** Same testing-scope conversation as `get_similar_incidents` earlier — constructing a genuinely ambiguous incident on demand isn't reliable, and isn't worth more real AI cost to chase. Ran two real tests instead (`analytics_failed` and `negative_balance_detected`, both previously well-understood, unambiguous bugs): both correctly stated a single hypothesis, no manufactured alternates, and the fix-proposal stage worked correctly off that single hypothesis in both cases — confirming the addition didn't degrade the common case. Whether ranking actually triggers for a genuinely ambiguous incident remains an honest, undemonstrated gap, same category as `get_similar_incidents` never having been observed in active use.

**Incidental finding, not a bug:** the `negative_balance_detected` test also fired a `background_task_failed` incident from the same burst (plausible — 12 orders at ~5% email failure each is roughly a 46% chance of at least one). The AI queue correctly processed both sequentially, one at a time, each getting its own full investigation — confirms the worker loop and dedup cooldown both continue to behave correctly under multiple co-occurring incident types, not just the single-event-type bursts tested earlier.

---

## 11. Live UI — Watching the Pipeline Instead of Reading Terminal Logs

**Why this exists at all.** Everything real this project has built — the cascade detection, the dedup cooldown, the vector memory, the hypothesis ranking — was only ever visible in `docker compose logs`. She found it genuinely hard to demo the system that way, and the engineering depth was real but invisible to anyone not scrolling raw terminal output. The UI's entire purpose is making that depth legible, not a cosmetic add-on.

### Architecture: the UI is sentinel-agent's own face, not a second system

First instinct was a separate decoupled service independently re-deriving state from shared resources (Redis, the log file) — rejected directly: "the UI should have ALL functioning of the sentinel AI agent." `sentinel-agent` already does the real work; the UI just needs a way to *see* it happening, not recompute it. `events.py` is a thread-safe in-memory log that every existing pipeline stage pushes into *alongside* its existing `print()` calls — nothing about current terminal behavior changed. A small FastAPI layer (`web.py`) was added directly to `sentinel-agent` itself, restructuring the entrypoint so uvicorn owns the main thread/process while the existing log-watcher loop and AI worker thread both move to background threads — same logic, different position in startup.

### Two streams, not one, found necessary before it became a real problem

Asked directly for "both working and non-working endpoints" from `target_app` — a real gap: `log_collector.py` only ever pushed events for *errors*, so the live feed had zero visibility into healthy activity. Pushing every log line into the same buffer as detection/AI events was flagged as a coming problem before building it: a burst of ordinary traffic (especially once a sustained-traffic trigger exists) would evict the low-volume, high-signal pipeline events a UI actually cares about. Split into two independently-bounded buffers in `events.py` — `pipeline` (detection → AI investigation → fix proposal) and `activity` (every `target_app` log line, success and failure) — with matching split endpoints in `web.py`.

### Incident correlation: one id threaded through everything

Asked for the layout to be "separated by target app triggers, error detector detects, then goes to first AI and then to 2nd AI" — chose a 4-column Kanban/CI-pipeline layout over a stacked-card-per-incident timeline specifically because it shows real concurrent throughput (multiple incidents mid-flight at once), not just one story at a time — closer to what a real production observability tool looks like, directly serving the "deep, not shallow" goal.

This required real correlation: a short id (`uuid.uuid4().hex[:8]`) generated the moment `handle_error()` detects an incident, threaded through every event that incident produces — into `ai_queue` (now carries `(incident_id, incident)` tuples), into `ai_engine.py` via a module-level `_current_incident_id` set once per `analyze_incident()` call and read by every tool's `_run()` and the stage callback. Safe as shared module state, not per-call, only because `ai_worker_loop` already processes incidents strictly one at a time — no two incidents are ever mid-investigation simultaneously. The frontend folds the flat pipeline stream into one record per incident id (`groupIncidents.js`), which is what makes "this tool call, this diagnosis, this fix all came from that trigger" visible instead of three unconnected lists.

### A second, disconnected id scheme, found and unified

`vector_memory.py` had its own internal id (`event_name:timestamp`) for stored incidents, completely separate from the live pipeline's id — two id schemes for the same incident, an inconsistency that existed since the vector memory work but only became a real problem once rating needed to attach a human judgment to a *specific* stored record. Unified: `store_incident()` now takes the live pipeline's id directly (via `upsert`, not `add`, defensive against being called twice for the same id).

### Outcome rating: this is the actual outcome-tracking data, not just a UI nicety

Asked directly "where can I score each fix it gives" — this connects straight back to the project's long-standing, repeatedly-cited blocker: the learned classifier, fix-accuracy benchmarking, and confidence calibration have all been stuck waiting on labeled "was this actually correct" data that didn't exist. `rate_incident()` records a correct/partial/incorrect judgment (plus optional note) against a stored incident's metadata — reads the existing metadata and writes back the full merged dict rather than trusting Chroma's `update()` to merge partial metadata on its own, since getting that assumption wrong would have silently destroyed the diagnosis/fix fields. Discussed whether 3 categories or a 1-10 score would serve this better — landed on 3: the downstream uses (classifier training, confidence calibration) want clean categorical labels, not a continuous score, and a 1-10 scale's extra granularity is mostly fake precision from a single rater doing quick spot-checks, not genuinely calibrated signal — while also being meaningfully higher friction per rating, which matters for whether the data actually gets collected at all.

### Real rendering bugs, found by actually looking, not assumed away

- **`react-markdown` v10 silently dropped the `inline` prop** the code component renderer used to receive (removed in a past major version) — the existing `if (inline)` check was therefore always false, so every code span, including a single inline word like `0` or `ZeroDivisionError`, went through the full block-level syntax-highlighter path instead of rendering as plain text. Fixed by checking for a `language-` className instead (only ever present on fenced block code, never on single-backtick inline code) — the actual distinguishing signal in this version.
- **Tool call outputs were being run through the markdown renderer meant for the AI's own prose.** `read_source_file`'s output is raw Python source, not markdown — passing it through `ReactMarkdown` collapsed all its newlines and indentation into one run-on paragraph (imports mashed together, a comment's leading `#` silently dropped), since markdown treats plain text with no code fences as ordinary prose. Tool outputs now render as plain preformatted text; only the investigator's and fixer's actual generated text goes through markdown.
- **Markdown headers in AI output used the browser's default heading sizes** — `.ai-output h1`-`h4` set margin and color but never constrained font-size, so a "## Root Cause" heading rendered 2-3x larger than the surrounding 14px body text.

### Thresholds tuned for a responsive UI, with the same reasoning already used elsewhere

`WARNING_THRESHOLD`/`INCIDENT_THRESHOLD` lowered from 3/5 to 1/3. The original values left every occurrence below 3 completely silent — no detector response at all until the 3rd one, which felt unresponsive during live testing (trigger an error, watch nothing happen in the UI for a while). 1 means the very first occurrence is immediately visible as at least a warning; 3 isn't an arbitrary new number, it matches the "two could be coincidence, three is a pattern" reasoning already used for `CASCADE_CONFIRMATION_THRESHOLD` in the same file, just applied consistently to AI dispatch too.

### Dockerized as a final step, not an afterthought
